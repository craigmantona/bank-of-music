(function initialiseBOMAlbumPresentation() {
  ...
  function trackRow(track) {
    ...
    return `<div class="track-row-table bom-v1-track-row${topClass}" data-track-index="${track.index}" data-song-id="${escapeHtml(track.songId || "")}">
      ...
      <div class="track-col-average" data-label="Community">${track.community ? `<strong>${track.community.average.toFixed(1)}</strong><span> / 10</span>` : "—"}</div>
      <div class="track-col-your-rating${track.personal !== null ? " has-rating" : ""}" data-label="You">${track.personal !== null ? `<strong>${track.personal}</strong><span> / 10</span>` : "—"}</div>
      <div class="track-col-stars">${track.ratingControlHtml || ""}</div>
  ...
  function tracks(model) {
    ...
    return `<section class="bom-v1-album-section bom-v1-tracks-section">
      ...
      <div class="bom-v1-track-table">
        <div class="bom-v1-track-header" aria-hidden="true"><span>#</span><span>Track</span><span>Time</span><span>Community</span><span>You</span><span></span><span></span></div>
  ...
  function render(model) {
    const meta = [
      model.releaseDate ? `<span>${escapeHtml(model.releaseDate)}</span>` : "",
  ...
  window.BOMAlbumUI = Object.freeze({ render, renderReviews });
})();
