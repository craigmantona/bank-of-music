-- Make shared MusicBrainz rate/cooldown coordination idempotently recoverable.
-- The singleton only needs the most recent receipt because the global lease
-- permits one catalogue worker and its requests are sequential.
begin;

alter table public.catalogue_worker_state
  add column coordination_request_token uuid,
  add column coordination_action text check (coordination_action in ('rate','cooldown')),
  add column coordination_not_before timestamptz,
  add column coordination_created_at timestamptz;
